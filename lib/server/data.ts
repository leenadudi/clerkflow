...
export async function listFoiaRequests(): Promise<FoiaRequest[]> {
  ...
  return rows.map((row) =>
    foiaToView(row, row.assignedUser, context.user?.id ?? null),
  )
}
...
export async function getFoiaRequest(publicId: string): Promise<FoiaRequest | null> {
  const townId = await getTownId()
  if (!townId) {
    return FOIA_REQUESTS.find((item) => item.id === publicId) ?? null
  }
...
export async function getMeeting(externalId: string): Promise<Meeting | null> {
  const townId = await getTownId()
  if (!townId) {
    return MEETINGS.find((item) => item.id === externalId) ?? null
  }
...
export async function updateFoiaStatus(publicId: string, status: StatusKey) {
  ...
  const [updated] = await db
    ...
    .where(
      and(eq(foiaRequests.townId, context.townId), eq(foiaRequests.publicId, publicId)),
    )
    .returning()
