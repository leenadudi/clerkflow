...
export default function FoiaQueuePage() {
  return (
    <div className="mx-auto max-w-7xl">
      ...
      {FOIA_REQUESTS.length === 0 ? (
        <div className="mt-6">
          <EmptyState
            icon={Inbox}
            title="No FOIA requests yet"
            description="Residents can submit public records requests at riverside-oh.clerkflow.software/foia"
          />
        </div>
      ) : (
        <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-5">
          {COLUMNS.map((col) => {
            const items = FOIA_REQUESTS.filter((r) => r.status === col.key)
            return (
