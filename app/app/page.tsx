...
import { FOIA_REQUESTS, MEETINGS, BOARD_TERMS, TOWN } from '@/lib/data'

export default function CommandCenterPage() {
  const overdue = FOIA_REQUESTS.filter((r) => r.status === 'overdue')
  const dueThisWeek = FOIA_REQUESTS.filter(
    (r) => r.daysRemaining >= 0 && r.daysRemaining <= 7,
  )
  const upcomingMeetings = MEETINGS.filter((m) => m.status === 'scheduled' || m.status === 'draft')
  const expiringTerms = BOARD_TERMS.filter((t) => t.expiringSoon)
